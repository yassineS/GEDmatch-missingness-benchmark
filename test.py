...
"""
    ...
    def test_remove_random_loci(self):
        ...
        with patch('random.sample', return_value=[0, 2]):  # Mock to return predictable indices
            ...
            self.assertEqual(df_downsampled.filter(pl.col("genotype") == "--").height, 3)  # 2 new + 1 existing
    ...
    def test_pseudo_haploidize_genotypes(self):
        ...
        with patch('random.randint', side_effect=[0, 1, 0, 1, 0]):  # Control random choices
            ...
            self.assertEqual(pseudo_df.filter(pl.col("rsid") == "rs1")["genotype"][0], "AA")  # first allele
            self.assertEqual(pseudo_df.filter(pl.col("rsid") == "rs2")["genotype"][0], "TT")  # homozygous already
            self.assertEqual(pseudo_df.filter(pl.col("rsid") == "rs3")["genotype"][0], "CC")  # homozygous already
            self.assertEqual(pseudo_df.filter(pl.col("rsid") == "rs4")["genotype"][0], "CC")  # second allele
            self.assertEqual(pseudo_df.filter(pl.col("rsid") == "rs5")["genotype"][0], "--")  # missing remains missing
