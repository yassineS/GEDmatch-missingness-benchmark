...
def main():
    ...
    for pct in percentages:
        ...
        try:
            process_diploid = subprocess.run(cmd_diploid, check=True, text=True, capture_output=True)
            ...
            row_data['Diploid Actual Missingness %'] = (missing_loci / total_loci * 100) if total_loci > 0 else 0
        ...
        try:
            process_pseudohap = subprocess.run(cmd_pseudohap, check=True, text=True, capture_output=True)
            print("Pseudo-haploid processing complete.")
            
            # Parse stats from output
            total_loci, missing_loci = extract_stats(process_pseudohap.stdout, "Pseudo-haploid stats")
            ...
            row_data['PseudoHaploid Actual Missingness %'] = (missing_loci / total_loci * 100) if total_loci > 0 else 0
